...
When('api user get first order detail', async function () {
    ...
    response = await axios.get(`https://staging-api-dashboard.superapp.co.id/api/order/detail/${this.first_id}`, {
        headers: {
            "Authorization": `Bearer ${this.token}`
        }
    })
...
When('api user get second order detail', async function () {
    ...
    response = await axios.get(`https://staging-api-dashboard.superapp.co.id/api/order/detail/${this.second_id}`, {
        headers: {
            "Authorization": `Bearer ${this.token}`
        }
    })
